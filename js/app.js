    ...
    (() => {
        ...
        function showMore() {
            window.addEventListener("load", (function(e) {
                ...
                function initItem(showMoreBlock, matchMedia = false) {
                    ...
                    const hiddenHeight = getHeight(showMoreBlock, showMoreContent) - 4;
                    const isMaxValue = getHeight(showMoreBlock, showMoreContent, true);
                    if (matchMedia.matches || !matchMedia) if (!isMaxValue && hiddenHeight < getOriginalHeight(showMoreContent)) {
                ...
                function getHeight(showMoreBlock, showMoreContent, checkMaxValue) {
                    let hiddenHeight = 0;
                    const showMoreType = showMoreBlock.dataset.showmore ? showMoreBlock.dataset.showmore : "size";
                    if (showMoreType === "items") {
                        const showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 3;
                        const showMoreItems = showMoreContent.children;
                        if (checkMaxValue) return showMoreItems.length <= showMoreTypeValue;
                ...
                function showMoreActions(e) {
                    ...
                    if (targetType === "click") {
                        if (targetEvent.closest("[data-showmore-button]")) {
                            ...
                            const hiddenHeight = getHeight(showMoreBlock, showMoreContent) - 4;
                            console.log(hiddenHeight, getOriginalHeight(showMoreContent));
            ...
            getErrors(form) {
                ...
                if (formRequiredItems.length) {
                    formRequiredItems.forEach((formRequiredItem => {
                        if ((formRequiredItem.offsetParent !== null || formRequiredItem.tagName === "SELECT" || formRequiredItem.hasAttribute("data-input-is-hidden")) && !formRequiredItem.disabled) error += this.validateInput(formRequiredItem);
                    }));
                    console.log(error);
                }
                return error;
        ...
        class SelectConstructor {
            constructor(props, data = null) {
                ...
                this.selectClasses = {
                    ...
                    classSelectOptionActive: "_active"
                };
                this._this = this;
                if (this.config.init) {
            ...
            getOption(selectOption, originalSelect) {
                const selectOptionSelectedOne = selectOption.selected || false;
                ...
                selectOptionHTML += selectOptionLink ? `<a ${selectOptionLinkTarget} ${selectOptionHide} href="${selectOptionLink}" data-value="${selectOption.value}" class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}">` : `<button ${selectOptionHide} class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}${selectOptionSelectedOne ? " " + this.selectClasses.classSelectOptionActive : ""}" data-value="${selectOption.value}" type="button">`;
            ...
            optionAction(selectItem, originalSelect, optionItem) {
                if (originalSelect.multiple) {
                    ...
                    if (originalSelect.hasAttribute("data-option-active") && originalSelect.hasAttribute("data-show-selected")) {
                        if (optionItem.parentElement.querySelector(`.${this.selectClasses.classSelectOption}.${this.selectClasses.classSelectOptionActive}`)) optionItem.parentElement.querySelector(`.${this.selectClasses.classSelectOption}.${this.selectClasses.classSelectOptionActive}`).classList.remove(`${this.selectClasses.classSelectOptionActive}`);
        ...
        function rangeInit() {
            ...
            if (rangeItems.length > 0) rangeItems.forEach((rangeItem => {
                ...
                const range = rangeItem.querySelector("[data-range-item]");
                const isTootip = range.hasAttribute("data-tooltip") || false;
                nouislider.create(range, {
                    start: [ Number(rangeFrom.value), Number(rangeTo.value) ],
                    connect: true,
                    format: formatForSlider,
                    tooltips: isTootip ? true : false,
                    range: {
                        min: [ Number(rangeFrom.dataset.rangeFrom) ],
                        max: [ Number(rangeTo.dataset.rangeTo) ]
                    }
                });
                isTootip ? mergeTooltips(range, 18, " - ") : null;
                rangeFrom.addEventListener("change", (e => {
                    setPriceValues(range, rangeFrom, rangeTo);
                }));
        ...
        function initSliders() {
            if (document.querySelector(".main-block__slider")) {
                ...
                new core(".main-block__slider", {
                    ...
                    on: {
                        init: function(mainSlider) {
                            allSlides.innerHTML = mainSlider.slides.length;
                            currentSlide.innerHTML = mainSlider.activeIndex < 10 ? `0${mainSlider.activeIndex + 1}` : mainSlider.activeIndex + 1;
                        },
                        slideChange: function(mainSlider) {
                            currentSlide.innerHTML = mainSlider.realIndex + 1 < 10 ? `0${mainSlider.realIndex + 1}` : mainSlider.realIndex + 1;
                        }
                    }
                });
            }
            ...
            if (document.querySelector(".images-product__slider") && document.querySelector(".images-product__slider")) {
        ...
        if (menuBlocks) menuBlocks.forEach((menuBlock => {
            const menuBlcoksItems = menuBlock.querySelectorAll(".sub-menu-catalog__category");
            menuBlcoksItems.length ? menuBlock.classList.add(`sub-menu-catalog__block_${menuBlcoksItems.length}`) : null;
        }));
        function documentActions(e) {
            const target = e.target;
            const subParent = target.closest("[data-subparent]");
            if (subParent) {
                e.preventDefault();
                const subMenuId = subParent.dataset.subparent ? subParent.dataset.subparent : null;
                if (subMenuId) {
                    const subMenu = document.querySelector(`[data-submenu="${subMenuId}"]`);
                    if (subMenu) {
                        const activeLink = document.querySelector("._menu-catalog-link-active");
                        const activeBlock = document.querySelector("._sub-menu-show");
                        if (activeLink && activeLink !== subParent) {
                            activeLink.classList.remove("_menu-catalog-link-active");
                            activeBlock.classList.remove("_sub-menu-show");
                        } else subMenu.closest(".sub-menu-catalog").classList.toggle("_sub-menu-open");
                        subParent.classList.toggle("_menu-catalog-link-active");
                        subMenu.classList.toggle("_sub-menu-show");
                    }
                } else console.log("Немає такого підменю");
            }
            if (target.closest(".menu-top-header__link_catalog")) {
                e.preventDefault();
                document.documentElement.classList.add("catalog-open");
            }
            if (target.closest(".icon-menu")) if (document.documentElement.classList.contains("catalog-open")) {
                document.documentElement.classList.remove("catalog-open");
                document.querySelector("._sub-menu-show") ? document.querySelector("._sub-menu-show").classList.remove("_sub-menu-show") : null;
                document.querySelector("._sub-menu-open") ? document.querySelector("._sub-menu-open").classList.remove("_sub-menu-open") : null;
                document.querySelector("._menu-catalog-link-active") ? document.querySelector("._menu-catalog-link-active").classList.remove("_menu-catalog-link-active") : null;
            }
            ...
            if (target.closest(".create-reviews__btn")) {
                const reviewForm = document.querySelector(".create-reviews__form");
                const alert = document.querySelector(".create-reviews__alert");
                reviewForm.classList.add("_show");
                alert.classList.add("_hide");
            }
        }
        window["FLS"] = true;
        isWebp();
